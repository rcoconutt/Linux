#! /bin/bash
# Install this sh*t
# Tested on Debian jessie, Linux Mint Betsy
...
function installAndroidStudio {
	sudo apt-get -y install lib32stdc++6 lib32z1 lib32ncurses5 lib32stdc++6 lib32gcc1 lib32tinfo5 libc6-i386 lib32bz2-1.0
	#In process...
	checkJava
...
deb-src http://ppa.launchpad.net/webupd8team/java/ubuntu trusty main' >> /etc/apt/sources.list 
	sudo apt-get update
...
function installVirtualbox {
	...
	wget http://download.virtualbox.org/virtualbox/5.0.20/Oracle_VM_VirtualBox_Extension_Pack-5.0.20-106931.vbox-extpack -O /tmp/Oracle_VM_VirtualBox_Extension_Pack-5.0.20-106931.vbox-extpack
	sudo VBoxManage extpack install /tmp/Oracle_VM_VirtualBox_Extension_Pack-5.0.20-106931.vbox-extpack
}
...
function installNginx {
	...
	#error_log '"$directorio"'logs/error.log;
	#access_log '"$directorio"'logs/access.log;
...
#}' >> /etc/nginx/sites-available/default
	...
	sudo apt-get -y install php5-fpm php5 php-pear php5-mysql php5-common php5-cli php5-mcrypt
...
function installWpscan {
	cd $home_path
	sudo apt-get -y install git ruby ruby-dev libcurl4-openssl-dev make zlib1g-dev
	git clone https://github.com/wpscanteam/wpscan.git
	sudo chmod -R 777 wpscan/
	cd wpscan
...
function installSublimeText {
	if [[ "$arch" == "x86_64" ]]; then
		...
		cd /tmp/
		sudo tar -jxf sublime.tar.bz2 --transform 's/Sublime Text 2/sublime-text-2/' -C /usr/lib/
	else
...
function installApache {
	sudo apt-get -y install mysql-server mysql-client apache2 apache2-doc php5-cgi php-pear
 	sudo a2enmod userdir

	sudo mysql_install_db
	sudo mysql_secure_installation
	
	sudo apt-get -y install php5 php5-mysql libapache2-mod-php5 perl libapache2-mod-perl2 python libapache2-mod-python curl
	sudo apt-get -y install phpmyadmin
}
...
function installGenymotion {
	#SID require
	sudo apt-get -y install libjpeg8 libjpeg8-dev libdouble-conversion-dbg libdouble-conversion-dev libdouble-conversion1 libxi-dev libxmu-dev libqt4-webkit libqt4-svg
}

function installWeevely {
	sudo apt-get -y install g++ python-pip libyaml-dev python-dev
	sudo pip install prettytable Mako PyYAML python-dateutil PySocks --upgrade
	cd $home_path
	git clone https://github.com/epinna/weevely3
	sudo chmod -R 777 weevely3/
}
